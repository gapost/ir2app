// Delta system constructor

function setR15offset(r)
{
	with(dev.nvm)
	{
		R1.offset=-r
...
function createDeltaControl(gpib,loop,dataBuffer)
{
	...
	with(figs.deltaCtrl)
	{
		...
		runButton.toggled.connect(deltaRunPressed);
...
function createDeltaFigs(Figs,Data)
{
	with(Figs)
	{
		with(Data)
...
function startDelta(srcCurrent)
{
	with(dev)
	{
...
function stopDelta()
{
	with(dev)
	{
		...
		ui = figs.deltaCtrl.widget();
		btRun = ui.findChild("Run");
		if (btRun.checked) btRun.checked=false;
	}
}

function deltaRunPressed(on)
{
	if (on)
	{
		ui = figs.deltaCtrl.widget();
